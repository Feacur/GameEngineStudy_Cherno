...
void Level::OnUpdate(GES::Timestep ts)
{
...
void Level::OnRender()
{
	...
	// Background
	Renderer2D::DrawQuad({ playerPos.x, 0.0f, -0.8f }, { 50.0f, 50.0f }, 0.0f, { 0.3f, 0.3f, 0.3f, 1.0f });

	// Floor and ceiling
	Renderer2D::DrawQuad({ playerPos.x,  34.0f }, { 50.0f, 50.0f }, 0.0f, color);
	Renderer2D::DrawQuad({ playerPos.x, -34.0f }, { 50.0f, 50.0f }, 0.0f, color);

	for (auto& pillar : m_Pillars)
	{
		Renderer2D::DrawQuad(pillar.TopPosition, pillar.TopScale, glm::radians(180.0f), color, m_TriangleTexture);
		Renderer2D::DrawQuad(pillar.BottomPosition, pillar.BottomScale, 0.0f, color, m_TriangleTexture);
	}
